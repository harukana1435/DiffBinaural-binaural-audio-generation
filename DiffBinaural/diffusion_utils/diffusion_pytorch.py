...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def ddim_sample(self, condition, return_all_timesteps = False, silence_mask_sampling = False, threshold = 2e-3, sampling_timesteps = None):
        device, total_timesteps, eta = self.betas.device, self.num_timesteps, self.ddim_sampling_eta #etaは1
        # sampling_timestepsパラメータが渡された場合はそれを使用、そうでなければデフォルト値を使用
