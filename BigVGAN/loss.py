...
class MultiScaleMelSpectrogramLoss(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        for n_mels, fmin, fmax, s in zip(
            ...
            losses.append(self.log_weight * self.loss_fn(x_logmels, y_logmels))
            losses.append(self.mag_weight * self.loss_fn(x_logmels, y_logmels))

        loss = sum(losses)  # Use sum() instead of += to avoid inplace operations
        return loss
...
def generator_loss(
    ...
    loss = sum(gen_losses)  # Use sum() instead of += to avoid inplace operations
